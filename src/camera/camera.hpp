...
public:
    // Constructors
    Camera(const glm::vec3& origin=glm::vec3(1.0f,0.0f,0.0f), float fov=90, float speed=1.5);
    Camera(const glm::vec3& origin, const glm::vec3& forward, const glm::vec3& up, float fov=90, float speed=1.5); //maybe normalize cord
    ...
    glm::mat4 perspectiveProjection() { return glm::perspective(glm::radians(fov), 800.0f / 600.0f, 0.1f, 100.0f); }
    glm::mat4 orthographicProjection() { return glm::ortho(0.0f, 800.0f, 0.0f, 600.0f, 0.1f, 100.0f); }
